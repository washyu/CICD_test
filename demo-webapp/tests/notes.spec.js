...
test.describe('Notes Application', () => {
  ...
  test('should delete a note', async ({ page }) => {
    ...
    // Add a new note first
    const noteText = 'Note to be deleted ' + Date.now();
    await page.locator('input[placeholder="Enter a new note"]').fill(noteText);
    await page.locator('button:has-text("Add Note")').click();
    ...
    // Find the delete button for this note and click it
    const noteItem = page.locator(`li:has-text("${noteText}")`).first();
    const deleteButton = noteItem.locator('button:has-text("Delete")');
    await deleteButton.click();

    // Wait for the note to disappear
    await expect(page.locator(`li:has-text("${noteText}")`)).toHaveCount(0);
  });
